...
- [ ] **Step 3: Implement models and constraints**
  ...
  M1's required unique `Company.isin` is the bounded identifier of the primary listed equity represented by the M1 `Company`/`Ticker` relationship. It is not SPA's permanent universal identifier for the corporate issuer. The required unique `ticker_id` and required, normalized-uppercase, unique `isin` behavior remain unchanged in M1, including one primary Ticker per Company. A later approved additive `Security`/`Listing` architecture may relocate or map this security identity while preserving `Company.id`, immutable research revisions, and existing historical relationships. This clarification does not authorize `ResearchSubject`, `Security`, `Listing`, multiple listings, or any schema change beyond the already approved M1 Company model.
...
- [ ] **Step 3: Implement revision models and command transaction**
  ...
  The stored immutable `ResearchRevision` is the authoritative M1 SPA Research View for that revision. Persist only the fields in the frozen M1 design; do not add a confidence field, `evidence_cutoff`, `evidence_cutoff_at`, a temporal evidence graph, or a methodology-version field unless it is already present in the frozen plan for another reason. `effective_at` retains its approved business-effective meaning and `created_at` retains its approved creation/audit meaning; neither may be reinterpreted as an evidence cutoff. A later AI-generated explanation, summary, or analysis must identify the exact stored revision and must not replace or silently rewrite it. Any future explicit evidence-cutoff model must be additive and must preserve historical M1 revisions.
